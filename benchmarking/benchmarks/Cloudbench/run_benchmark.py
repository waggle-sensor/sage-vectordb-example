"""Run CloudBench benchmark: load data and evaluate queries."""
...
from data_loader import CloudBenchDataLoader

config = CloudBenchConfig()


def load_data(
    data_loader: CloudBenchDataLoader,
...
def run_evaluation(evaluator: BenchmarkEvaluator, hf_dataset: Dataset):
    """Run the CloudBench benchmark evaluation."""
...
def main():
    ...
    benchmark_dataset = CloudBench(dataset_name=config.cloudbench_dataset)
    # CloudBench has only "train" split on HuggingFace
