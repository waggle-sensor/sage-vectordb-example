"""Run CommonObjectsBench benchmark: load data and evaluate queries."""
...
from benchmark_dataset import CommonObjectsBench
from config import CommonObjectsBenchConfig
from data_loader import CommonObjectsBenchDataLoader

config = CommonObjectsBenchConfig()


def load_data(
    data_loader: CommonObjectsBenchDataLoader,
    vector_db: VectorDBAdapter,
    hf_dataset,
):
    """Load CommonObjectsBench dataset into Weaviate."""
    try:
        ...
        logging.info(
            f"Successfully loaded {config.commonobjectsbench_dataset} into Weaviate collection '{config._collection_name}'"
        )
...
def run_evaluation(evaluator: BenchmarkEvaluator, hf_dataset):
    """Run the CommonObjectsBench benchmark evaluation."""
...
def main():
    ...
    benchmark_dataset = CommonObjectsBench(
        dataset_name=config.commonobjectsbench_dataset
    )
